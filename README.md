...
| Use Case (from the brief, verbatim) | Status | Pehra Feature & Implementation | Technical Architecture | Verification |
|---|---|---|---|---|
| **1. Simplifying complex legal documents** | **✅ Solid** | Generates clear, plain-language summaries and clause explanations in **English, Hindi, and Bengali**, stripping dense legalese into everyday actionable terms. | Gemini 2.5 Flash via `@google/genai` with strict zero-jargon system instructions (`src/lib/gemini.ts`). | Multilingual UI tests (`tests/ui.test.tsx`) |
| **2. Comparing contracts, agreements, or policies** | **✅ Solid** | **Contract Comparison Mode**: Side-by-side comparison between original agreements and renewal/amendment versions (e.g., lease renewals, employment amendments). Highlights new obligations, removed protections, and overall risk delta. | `src/lib/compare.ts`, `POST /api/compare`, and `ComparisonView.tsx` with schema-enforced change classification. | `tests/compare.test.ts` (6 tests) |
| **3. Highlighting clauses, obligations, risks, inconsistencies** | **✅ Solid** | **Internal Inconsistency Engine & Risk Flagging**: Checks clause-against-clause contradictions (e.g., notice rights vs instant forfeiture) alongside 4 strict verdicts: `void` (unenforceable), `one_sided` (oppressive), `standard` (ordinary), and `missing` (omitted rights). | `inconsistencies` in `src/lib/schema.ts`, `src/lib/gemini.ts`, and `src/app/page.tsx` mapped to 15 Indian statutory provisions (`src/lib/corpus/statutes.ts`). | `tests/grounding.test.ts` (14 tests), `tests/route.test.ts` |
| **4. Answering questions based on provided legal documents** | **✅ Solid** | **Interactive Document Q&A ("Ask Pehra")**: Ask specific questions about deposit refunds, notice periods, or non-competes. Features 1-click suggested prompts and freeform queries strictly grounded in document text. | `src/lib/qa.ts`, `POST /api/ask`, and `DocumentQA.tsx` strictly grounded in the document text and statutory corpus. | `tests/qa.test.ts` (5 tests) |
| **5. Helping users understand options and next steps** | **✅ Solid** | **Explicit 'Your Options & Potential Next Steps' Engine**: Actionable decision matrix categorizing concrete paths: redlining/negotiation, dispute redressal (Consumer Commission s. 35), and free legal aid (NALSA s. 12). Coupled with deterministic limitation clocks. | `optionsAndNextSteps` in `src/lib/schema.ts` + `src/lib/clock.ts` limitation engine (CPA 2019, NI Act s. 138). | `tests/clock.test.ts` (30 tests), `tests/route.test.ts` |
| **6. Generating summaries, checklists, or actionable outputs** | **✅ Solid** | **Literal Interactive Checklist & Downloadable Briefing**: Interactive checkable legal to-do list with real-time progress tracking, plus one-click **"Download Legal Briefing" (.txt)** and **"Print for Lawyer"** formatted for DLSA/NALSA. | `actionableChecklist` in `src/lib/schema.ts`, `downloadBrief()` client generator, and `@media print` legal intake stylesheet. | `tests/ui.test.tsx`, `tests/route.test.ts` |
| **7. Preparing questions for a legal professional** | **✅ Solid** | Generates case-tailored questions for the user to present at their District Legal Services Authority (DLSA) or via the NALSA 15100 helpline under s. 12 of the Legal Services Authorities Act, 1987. | `questionsForALawyer` output field, routing ordinary citizens directly to free state-guaranteed legal aid. | `tests/route.test.ts` |
